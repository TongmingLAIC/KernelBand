...
class performance_metrics(Performance_Metrics):
    ...
    def get_input_tensors(self):
        self.input_tensors = []
        # Original issue was k shape (B, H, K, T) instead of correct (B, H, T, K)
        # With correct k shape, the full range should work
        for i in range(2, 12):  # T from 4 to 2048 (original range)
            B, H, T, K, V = 8, 8, 2**i, 32, 32  # Example dimensions
            q = torch.rand(B, H, T, K, dtype=torch.float32)
            # k shape must be (B, H, T, K) to match kernel expectation
            # The kernel accesses k with strides (1, K) expecting row-major (T, K) layout
