...
@triton.jit
def tanh(x):
    """Tanh activation function - platform independent"""
    # Use tl.math.tanh or manual implementation: (exp(2x) - 1) / (exp(2x) + 1)
    exp2x = tl.exp(2.0 * x)
...
@triton.jit
def _approx_erf(x):
    """Approximate erf using Abramowitz and Stegun formula (platform independent)"""
...
@triton.jit
def gelu(x):
    """Gaussian Error Linear Unit (GELU) - platform independent"""
    return x * 0.5 * (1.0 + _approx_erf(x / sqrt2))
