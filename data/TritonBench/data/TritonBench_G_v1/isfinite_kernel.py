...
# Use triton standard math functions instead of platform-specific libdevice
# This works on both NVIDIA and AMD GPUs

@triton.jit
def _tl_isinf(x):
    """Check if value is infinity (works on both NVIDIA and AMD)."""
    return tl.abs(x) == float('inf')

@triton.jit
def _tl_isnan(x):
    """Check if value is NaN (works on both NVIDIA and AMD)."""
    return x != x
...
@triton.jit
def isfinite_func(x):
    # Use platform-independent implementations
    return ~_tl_isinf(x) & ~_tl_isnan(x)
