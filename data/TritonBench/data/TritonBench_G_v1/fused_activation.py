...
def fused_add_mul_activation_kernel(x_ptr, bias_ptr, in_ptr,
    ...
    if activation == "sigmoid":
        # tl.sigmoid doesn't exist, use manual implementation
        # tl.math.exp only supports fp32/fp64, so convert first
        activ_f32 = activ_input.to(tl.float32)
        neg_input = -activ_f32
        ma_result = (1.0 / (1.0 + tl.math.exp(neg_input))).to(activ_input.dtype)
