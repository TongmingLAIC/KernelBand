...
def token_att_fwd2(prob, v, out, Req_to_tokens, B_req_idx, B_Start_Loc, B_Seqlen):
    ...
    kv_group_num = prob.shape[0] // v.shape[0]  # num_heads // kv_heads
...
def test_token_att_fwd2():
    ...
    kv_heads = 4   # kv heads (kv_group_num = num_heads // kv_heads = 1)
    seq_len = 128
    d_model = 64

    # Create random input tensors
    # prob shape: (num_heads, total_seq_len) where total_seq_len >= batch_size * seq_len
    prob = torch.rand((num_heads, batch_size * seq_len), dtype=torch.float32, device='cuda')
    # v shape: (kv_heads, total_seq_len, d_model)
    v = torch.rand((kv_heads, batch_size * seq_len, d_model), dtype=torch.float32, device='cuda')
    out = torch.zeros((batch_size, num_heads, d_model), dtype=torch.float32, device='cuda')
    Req_to_tokens = torch.randint(0, batch_size * seq_len, (batch_size, seq_len), dtype=torch.int32, device='cuda')
    B_req_idx = torch.arange(batch_size, dtype=torch.int32, device='cuda')
    B_Start_Loc = torch.arange(batch_size, dtype=torch.int32, device='cuda') * seq_len
    ...
    seq_len_2 = 64
    prob_2 = torch.rand((num_heads, batch_size * seq_len_2), dtype=torch.float32, device='cuda')
    v_2 = torch.rand((kv_heads, batch_size * seq_len_2, d_model), dtype=torch.float32, device='cuda')
    out_2 = torch.zeros((batch_size, num_heads, d_model), dtype=torch.float32, device='cuda')
    Req_to_tokens_2 = torch.randint(0, batch_size * seq_len_2, (batch_size, seq_len_2), dtype=torch.int32, device='cuda')
    B_Start_Loc_2 = torch.arange(batch_size, dtype=torch.int32, device='cuda') * seq_len_2
    B_Seqlen_2 = torch.full((batch_size,), seq_len_2, dtype=torch.int32, device='cuda')

    token_att_fwd2(prob_2, v_2, out_2, Req_to_tokens_2, B_req_idx, B_Start_Loc_2, B_Seqlen_2)
    torch.cuda.synchronize()
    result["test_case_2"] = out_2.clone()

    # Test case 3: Different batch size
    batch_size_3 = 3
    prob_3 = torch.rand((num_heads, batch_size_3 * seq_len), dtype=torch.float32, device='cuda')
    v_3 = torch.rand((kv_heads, batch_size_3 * seq_len, d_model), dtype=torch.float32, device='cuda')
    out_3 = torch.zeros((batch_size_3, num_heads, d_model), dtype=torch.float32, device='cuda')
    Req_to_tokens_3 = torch.randint(0, batch_size_3 * seq_len, (batch_size_3, seq_len), dtype=torch.int32, device='cuda')
    B_req_idx_3 = torch.arange(batch_size_3, dtype=torch.int32, device='cuda')
    B_Start_Loc_3 = torch.arange(batch_size_3, dtype=torch.int32, device='cuda') * seq_len
